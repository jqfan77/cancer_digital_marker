...
def predict( genefile_filename, select_dataset, tissue_selected_id_list, drug_id_list ):
    ...
    # gene list for prediction
    # genes recorded in X_test
    ...
    for iii in range(len(drug_id_list)):
        ...
        # transfer to relative zscores among the predicted cell lines
        Y_mean = np.mean(Y)
        Y_std = np.std(Y)
        Y_predict_relative = ( Y_predict - Y_mean ) / Y_std
        # show the color, red for non-sensitive, green for sensitive, black for neutral
        the_color = '#000000'
